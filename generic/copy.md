# Generic — Landing Page Copy (template-mother)

> **URL:** `/generic/` (reached from the root hub `index.html`).
> **Template:** A (operator pitch) — **Movement → Control → Gains.**
> **Status:** CANONICAL 2026-06-16 — de-verticalized from `copy/automotive.md` (Feedback-04 canonical). This is the **industry-agnostic base** and the template-mother for Entertainment / Sports / future verticals. HTML mirrors this file verbatim per `feedback_copy_html_sync`.
> **Audience:** **Any business with recurring customers and meaningful card volume** — retail, hospitality, services, health & wellness, multi-location operators, franchises. Industry-neutral by design (named industries get their own dedicated pages).
...
## De-verticalization notes (read first)
...
| Field | Value |
|---|---|
| Title (browser tab) | `CredX — Own Your Customer, Recover $30K per $1M` |
| Meta description | `Keep the customer, the data, and the loyalty your business earns. Recover up to $30,000 for every $1M you process. CredX is the embedded value platform for businesses with recurring customers and card volume.` |
| H1 | See §1 Hero headline. |
| OG title | `CredX — Own Your Customer, Recover $30K per $1M` |
...
| OG image | ⚠ placeholder (reuses automotive OG) — regen for a neutral scene. |
...
**Status:** CANONICAL
**Beat:** Movement hook. Static cinematic hero image (video slot deferred). Primary CTA anchor-scrolls to the §8 form.

| Field | Value |
|---|---|
| Eyebrow | `For Operators Who Earn Their Customers` |
| Headline (H1) | `Own the Customer You Fought to Win` |
| Subheadline | `Every transaction, every repeat visit, every sale builds a relationship. CredX keeps it yours: the data, the loyalty, and the next visit.` |
| Primary CTA | `Join the Movement` → scrolls to §8 form |
| Secondary CTA | `See how it works` → scrolls to §6 How It Works |
| Hero asset | Static cinematic image. No play button, no video modal this round. ⚠ placeholder image. |
| Hero alt text | `A business counter where a branded credit line is issued to a returning customer` |

**Notes:**
- The `$30,000 per $1M` figure stays out of the hero subhead — it lives only in the §4 outcomes strip. Movement first, money is the cherry.
- Accent magenta on a key fragment is colour-only, no italic.
- **Nav carries two CTAs:** `Book a Demo` (secondary) + `Join the Movement` (primary, dominant). Book a Demo is a distinct destination (Calendly, pending Kyle) — falls back to the §8 form anchor until the URL lands. Hidden below 640px.
...
> **Build note:** render as collapsed expanders (accordion) — headline visible, body on click. Three voices, one closing line.
...
**Status:** CANONICAL
**Beat:** The three things the operator gets back + the white-label beat. Written as a gain, never a warning.
...
| Item | Copy |
|---|---|
| Your customer. | `The relationship stays in your business's name, not a competing credit facility's. The customer who walked in last Tuesday belongs to you.` |
...
| Keep the fence. | `A closed-loop value-back program in your own brand that keeps the customer from taking their business down the street.` |
...
- The interchange numbers ($36K competing / $6K CredX / $30K saved) are confirmed correct (user, 2026-06-09).
...
*(Single-output, same as Automotive.)*

> **Build note (UX blueprint):** moving the slider should reveal / pulse the inline form below it. Output animates count-up on change.
...
|---|---|
| Eyebrow | `Built For` |
| Headline | `Built for any business with recurring customers and card volume.` |
...
- `Type of business` ( `Retail & e-commerce` · `Hospitality & food` · `Health & wellness` · `Professional & home services` · `Multi-location / franchise` · `Other` )
...
**Build note (operator-facing):** Step 1 fields feed Kyle's CRM the moment they are submitted, even if Step 2 is skipped. Never show more than three or four fields in a single view.
...
| `What types of businesses does this work for?` | `Any business with recurring customers and meaningful card volume: retail and e-commerce, hospitality and food service, health and wellness, professional and home services, membership businesses, and multi-location operators. If your customers come back and pay by card, CredX applies.` |
...
| `Who qualifies?` | `Businesses processing $250K or more per month in card volume. Single-location and multi-location businesses both qualify — the savings and data benefits are proportional to your volume.` |
...
**Note:** no footer tagline (inherited from Feedback-04 C1).
...
| Section | Status |
|---|---|
| Page metadata (campaign, not indexed) | CANONICAL — industry-neutral audience |
| 1 · Hero — The Movement | CANONICAL — generic hook; video deferred; ⚠ placeholder image |
| 2 · The Movement — Three Voices | CANONICAL — 3-voice block kept verbatim |
...
**Downstream:** HTML mirrors this file verbatim (`feedback_copy_html_sync`). Imagery is placeholder (reuses Automotive) — regen for neutral scenes (Phase 5). This file is the template-mother for Entertainment / Sports.
